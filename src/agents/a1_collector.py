...
class A1Collector(ExecutionAgent):
    ...
    async def run(self, state: dict) -> dict:
        ...
        async with httpx.AsyncClient(timeout=timeout) as client:
            for src in sources:
                try:
                    entries = await self._fetch_source(client, src, defaults)
                    entries = entries[:top_n]
                    hot_raw.extend(entries)
                    source_stats[src["id"]] = len(entries)
                    self.log(f"  {src['name']}: 获取 {len(entries)} 条")
                except Exception as e:
                    source_stats[src["id"]] = f"error: {e}"
        ...
        self.log(f"热点采集完成，共 {len(hot_raw)} 条")
        return state
    ...
    async def _fetch_source(self, client: httpx.AsyncClient, src: dict,
        ...
        return self._normalize(resp.json(), src)
    ...
    def _normalize(self, data, src: dict) -> list[dict]:
    ...
    def _write_output(self, state: dict, source_stats: dict):
        """A1 自行落盘 hot_raw.json（含 meta 信封）"""
        config = self._load_config()
        ...
        payload = {
            "meta": {
                ...
                "status": "ok" if state.get("hot_raw") else "error",
            },
            "source_stats": source_stats,
            "items": state.get("hot_raw", []),
        }
        out_path.write_text(json.dumps(payload, ensure_ascii=False, indent=2),
                            encoding="utf-8")
        self.log(f"已落盘 {out_path}（{len(payload['items'])} 条）")
